...
ノードどうしの接続を設定する場合は、 setInput メソッドを使用します。
...
Python から hscript を実行するには hou.hscript 関数を使用します。
...
.. function:: hou.hscript(command_str)

    hscript のコマンドとして実行できる文字列を渡すと実行され、標準出力と標準エラーの結果を文字列で返します
